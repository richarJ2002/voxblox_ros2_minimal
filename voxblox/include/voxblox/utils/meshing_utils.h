...
bool getSdfIfValid(const VoxelType &voxel, const float min_weight, float *sdf);

template <>
inline bool
    getSdfIfValid(const TsdfVoxel &voxel, const float min_weight, float *sdf)
{
    DCHECK(sdf != nullptr);
...
template <typename VoxelType>
bool getColorIfValid(const VoxelType &voxel,
                     const float      min_weight,
                     Color           *color);
