...
void centerBlocksOfLayer(Layer<VoxelType> *layer, Point *new_layer_origin)
{
    ...
    centroid /= static_cast<float>(block_indices.size());
    ...
    *new_layer_origin      = index_centroid.cast<float>() * block_size;
    ...
    // hash map and re-insert them again. This sounds worse than it is, the
    // blocks are all shared ptrs and therefore only a negligible amount of real
    // memory operations is necessary.
