...
class CameraModel
{
  public:
    ...
    void setIntrinsicsFromFocalLength(
        const Eigen::Matrix<float, 2, 1> &resolution,
        double                            focal_length,
        double                            min_distance,
        double                            max_distance);
    void setIntrinsicsFromFoV(double horizontal_fov,
                              double vertical_fov,
                              double min_distance,
                              double max_distance);
  ...
  private:
    ...
    /**
     * The original vertices of the frustum, in the axis-aligned coordinate
     * frame (before rotation).
     */
    AlignedVector<Point> corners_C_;

    /**
     * The 6 bounding planes for the current camera pose, and their
     * corresponding AABB (Axis Aligned Bounding Box). Expressed in global
     * coordinate frame.
     */
