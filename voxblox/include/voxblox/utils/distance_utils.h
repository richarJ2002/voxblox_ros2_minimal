                                ...
                                Point                  *triangulated_pose)
{
    ...
    while (t < max_distance)
    {
        ...
        if (!block_ptr)
        {
            // How much should we move up by? 1 voxel? 1 block? Could be close
            // to the block boundary though.... Let's start with the naive
            // choice: 1 voxel.
            t += voxel_size;
            continue;
        }
        ...
        // TODO(helenol): these are gross generalizations; use actual
        // interpolation to get the surface boundary.
