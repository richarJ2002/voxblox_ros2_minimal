...
enum class VoxelEvaluationResult
{
    kNoOverlap,
    kIgnored,
    kEvaluated
...
VoxelEvaluationResult
    computeVoxelError(const VoxelType          &voxel_gt,
                      const VoxelType          &voxel_test,
                      const VoxelEvaluationMode evaluation_mode,
                      float                    *error);

/// Returns true if the voxel has been observed.
template <typename VoxelType> bool isObservedVoxel(const VoxelType &voxel);

/// Allow this class to be templated on all kinds of voxels.
template <typename VoxelType> float getVoxelSdf(const VoxelType &voxel);

template <typename VoxelType>
void setVoxelSdf(const float sdf, VoxelType *voxel);

template <typename VoxelType>
void setVoxelWeight(const float weight, VoxelType *voxel);
 ...
 */
template <typename VoxelType>
float evaluateLayersRmse(const Layer<VoxelType>    &layer_gt,
                         ...
                         Layer<VoxelType>          *error_layer       = nullptr)
{
    ...
    for (const BlockIndex &block_index : block_list)
    {
        ...
        const Block<VoxelType> &gt_block =
            layer_gt.getBlockByIndex(block_index);
    ...
    {
        const Block<VoxelType> &gt_block =
            layer_gt.getBlockByIndex(gt_block_index);
                         ...
                         const Layer<VoxelType> &layer_test)
{
    return evaluateLayersRmse<VoxelType>(
        layer_gt,
        layer_test,
        VoxelEvaluationMode::kIgnoreErrorBehindTestSurface);
}
...
template <> bool  isObservedVoxel(const TsdfVoxel &voxel);
template <> bool  isObservedVoxel(const EsdfVoxel &voxel);
template <> float getVoxelSdf(const TsdfVoxel &voxel);
template <> float getVoxelSdf(const EsdfVoxel &voxel);
template <> void  setVoxelSdf(const float sdf, TsdfVoxel *voxel);
template <> void  setVoxelSdf(const float sdf, EsdfVoxel *voxel);
template <> void  setVoxelWeight(const float weight, TsdfVoxel *voxel);
template <> void  setVoxelWeight(const float weight, EsdfVoxel *voxel);
