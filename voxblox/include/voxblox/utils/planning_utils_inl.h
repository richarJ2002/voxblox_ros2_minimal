...
void getSphereAroundPoint(const Layer<VoxelType> &layer,
                          const Point            &center,
                          float                   radius,
                          HierarchicalIndexMap   *block_voxel_list)
{
    ...
    const float radius_in_voxels = radius / voxel_size;
            ...
            for (float z = -radius_in_voxels; z <= radius_in_voxels; z++)
            {
                ...
                if (point_voxel_space.norm() <= radius_in_voxels)
                {
                    ...
                    getBlockAndVoxelIndexFromGlobalVoxelIndex(
                        voxel_offset_index + center_index,
                        voxels_per_side,
                        &block_index,
                        &voxel_index);
...
template <typename VoxelType>
void getAndAllocateSphereAroundPoint(const Point          &center,
                                     float                 radius,
                               ...
                               Eigen::Vector3d                 *upper_bound)
{
    ...
    *lower_bound = getOriginPointFromGridIndex(lower_bound_index, block_size)
                       .cast<double>();
    *upper_bound =
