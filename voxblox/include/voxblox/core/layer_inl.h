...
template <typename VoxelType> Layer<VoxelType>::Layer(const Layer &other)
{
         ...
         other.block_map_)
    {
             ...
             ++linear_idx)
        {
            const VoxelType &voxel =
                block_ptr->getVoxelByLinearIndex(linear_idx);
                                  ...
                                  bool               clear_file) const
{
    ...
    return saveSubsetToFile(file_path,
                            BlockIndexList(),
                            kIncludeAllBlocks,
                            clear_file);
}
        ...
        for (const BlockMapPair &pair : block_map_)
        {
            ...
            if (!write_block_to_file)
            {
                BlockIndexList::const_iterator it =
                    std::find(blocks_to_include.begin(),
                              blocks_to_include.end(),
                              pair.first);
                                         ...
                                         BlockMergingStrategy strategy)
{
    ...
    if (isCompatible(block_proto))
    {
        ...
        const BlockIndex        block_index =
            getGridIndexFromOriginPoint<BlockIndex>(block_ptr->origin(),
                                                    block_size_inv_);
        switch (strategy)
        {
    ...
    {
        LOG(ERROR) << "The blocks from this protobuf are not compatible with "
                      "this layer!";
        return false;
    }
    return true;
...
bool Layer<VoxelType>::isCompatible(const LayerProto &layer_proto) const
{
    ...
    if (!compatible)
    {
        LOG(WARNING) << "Voxel size of the loaded map is: "
                     << layer_proto.voxel_size()
                     << " but the current map is: " << voxel_size_
                     << " check passed? "
                     << (std::fabs(layer_proto.voxel_size() - voxel_size_) <
                         std::numeric_limits<float>::epsilon())
                     << "\nVPS of the loaded map is: "
                     << layer_proto.voxels_per_side()
...
template <typename VoxelType> size_t Layer<VoxelType>::getMemorySize() const
{
    size_t size = 0u;
...
template <typename VoxelType> std::string Layer<VoxelType>::getType() const
{
