 ...
 */
template <typename VoxelType> class Layer
{
  public:
    ...
    typedef typename AnyIndexHashMapType<typename BlockType::Ptr>::type
        BlockHashMap;
    typedef typename std::pair<BlockIndex, typename BlockType::Ptr>
        BlockMapPair;
        ...
        voxels_per_side_(voxels_per_side)
    {
        ...
        voxels_per_side_inv_ = 1.0f / static_cast<float>(voxels_per_side_);
    }
    ...
    inline typename BlockType::ConstPtr
        getBlockPtrByIndex(const BlockIndex &index) const
    {
        typename BlockHashMap::const_iterator it = block_map_.find(index);
        if (it != block_map_.end())
        {
    ...
    /**
     *  Gets a block by the block index it if already exists, otherwise
     * allocates a new one.
     */
    inline typename BlockType::Ptr
        allocateBlockPtrByIndex(const BlockIndex &index)
    {
        typename BlockHashMap::iterator it = block_map_.find(index);
        if (it != block_map_.end())
        {
    ...
    inline typename BlockType::ConstPtr
        getBlockPtrByCoordinates(const Point &coords) const
    {
     ...
     */
    inline typename BlockType::Ptr
        allocateBlockPtrByCoordinates(const Point &coords)
    {
        return allocateBlockPtrByIndex(
            computeBlockIndexFromCoordinates(coords));
    }

    /**
     * IMPORTANT NOTE: Due the limited accuracy of the float type, this
    ...
    inline typename BlockType::Ptr
        allocateNewBlockByCoordinates(const Point &coords)
    {
            ...
            &block_pair)
    {
        ...
        DCHECK(insert_status.second)
            << "Block already exists when inserting at "
     ...
     * nullptr otherwise.
     */
    inline const VoxelType *
        getVoxelPtrByGlobalIndex(const GlobalIndex &global_voxel_index) const
    {
        const BlockIndex block_index =
            getBlockIndexFromGlobalVoxelIndex(global_voxel_index,
    ...
    inline VoxelType *
        getVoxelPtrByGlobalIndex(const GlobalIndex &global_voxel_index)
    {
        const BlockIndex block_index =
            getBlockIndexFromGlobalVoxelIndex(global_voxel_index,
                                              voxels_per_side_inv_);
    ...
    float voxels_per_side_inv() const
    {
