...
template <typename VoxelType> class Block
{
  public:
     ...
     * index that is within this block if you pass a coordinate outside the
     * range of this block. Try not to use this function if there is an
     * alternative to directly address the voxels via precise integer indexing
     * math.
     */
    VoxelIndex
        computeTruncatedVoxelIndexFromCoordinates(const Point &coords) const;

    /**
     * NOTE: This function is also dangerous, use in combination with
     * Block::isValidVoxelIndex function.
     * This function doesn't truncate the voxel index to the [0,
     * voxels_per_side] range when the coordinate is outside the range of this
     * block, unlike the function above.
     */
    inline VoxelIndex
        computeVoxelIndexFromCoordinates(const Point &coords) const
    {
        VoxelIndex voxel_index =
            getGridIndexFromPoint<VoxelIndex>(coords - origin_,
                                              voxel_size_inv_);
        return voxel_index;
    }
    ...
    {
        return getGridIndexFromOriginPoint<BlockIndex>(origin_,
                                                       block_size_inv_);
    }
    ...
    const std::bitset<Update::kCount> &updated() const
    {
        return updated_;
    }
    std::bitset<Update::kCount> &updated()
    {
        return updated_;
    }
    bool &has_data()
  ...
  protected:
    ...
    /// Is set to true if any one of the voxels in this block received an
    /// update.
    bool has_data_;
