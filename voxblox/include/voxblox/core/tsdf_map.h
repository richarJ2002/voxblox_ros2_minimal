...
class TsdfMap
{
  public:
        ...
        interpolator_(tsdf_layer_.get())
    {
        if (!layer)
        {
            /* NOTE(mereweth@jpl.nasa.gov) - throw std exception for Python to
             * catch This is idiomatic when wrapping C++ code for Python,
             * especially with pybind11
             */
