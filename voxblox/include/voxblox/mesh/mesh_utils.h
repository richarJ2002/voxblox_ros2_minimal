    ...
    const float approximate_vertex_proximity_threshold = 1e-10)
{
    ...
    for (const Mesh::ConstPtr &mesh : meshes)
    {
        ...
        // Stores the mapping from old vertex index to the new one in the
        // combined mesh. This is later used to adapt the triangles to the new,
        // global indexing of the combined mesh.
             ...
             ++old_vertex_idx)
        {
            ...
            const LongIndex vertex_3D_index =
                LongIndex(std::round(scaled_vector.x()),
                          std::round(scaled_vector.y()),
                          std::round(scaled_vector.z()));

            // If the current vertex falls into the same grid cell as a previous
            // vertex, we merge them. This is done by assigning the new vertex
            // to the same vertex index as the first vertex that fell into that
            // cell.
            ...
            if (vertex_is_unique_so_far)
            {
                ...
                if (mesh->hasColors())
                {
                    ...
                    connected_mesh->colors.push_back(
                        mesh->colors[old_vertex_idx]);
                }
                if (mesh->hasNormals())
                {
                    CHECK_LT(old_vertex_idx, mesh->normals.size());
                    connected_mesh->normals.push_back(
                        mesh->normals[old_vertex_idx]);
                }

                // Store the new vertex index in the unique-vertex-map to be
                // able to retrieve this index later if we encounter vertices
                // that are supposed to be merged with this one.
                CHECK(
                    uniques.emplace(vertex_3D_index, new_vertex_index).second);

                // Also store a mapping from old index to new index for this
                // mesh block to later adapt the triangle indexing.
            ...
            {
                // If this vertex is not unique, we map it's vertex index to the
                // new vertex index.
                CHECK_LT(old_vertex_idx, old_to_new_indices.size());
                old_to_new_indices[old_vertex_idx] = it->second;

                // Add all normals (this will average them once they are
                // renormalized later)
                connected_mesh->normals[it->second] +=
                    mesh->normals[old_vertex_idx];
            }
