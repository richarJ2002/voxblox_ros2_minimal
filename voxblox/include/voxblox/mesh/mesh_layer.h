...
class MeshLayer
{
  public:
    ...
    explicit MeshLayer(float block_size) :
        block_size_(block_size),
        block_size_inv_(1.0 / block_size)
    {}
    virtual ~MeshLayer() {}
    ...
    inline typename Mesh::ConstPtr
        getMeshPtrByIndex(const BlockIndex &index) const
    {
        typename MeshMap::const_iterator it = mesh_map_.find(index);
        if (it != mesh_map_.end())
        {
    ...
    inline typename Mesh::ConstPtr
        getMeshPtrByCoordinates(const Point &coords) const
    {
    ...
    /// Coord to mesh index.
    inline BlockIndex
        computeBlockIndexFromCoordinates(const Point &coords) const
    {
    ...
    typename Mesh::Ptr allocateNewBlock(const BlockIndex &index)
    {
        auto insert_status = mesh_map_.insert(std::make_pair(
            index,
            std::shared_ptr<Mesh>(
                new Mesh(block_size_, index.cast<float>() * block_size_))));
    ...
    {
        // we clear the mesh, but do not delete it from the map as the empty
        // mesh must be sent to rviz so it is also cleared there
    ...
    /**
     * Get mesh from mesh layer. NOTE: The triangles and vertices in this mesh
     * are distinct, hence, this will not produce a connected mesh.
     */
    void getMesh(Mesh *combined_mesh) const
    {
        ...
        // Check if color, normals and indices are enabled for the first
        // non-empty mesh. If they are, they need to be enabled for all other
        // ones as well.
        ...
        // Loop again over all meshes to figure out how big the mesh needs to
        // be.
        size_t mesh_size = 0;
        for (const BlockIndex &block_index : mesh_indices)
        {
        ...
        for (const BlockIndex &block_index : mesh_indices)
        {
            ...
            // Check assumption that all meshes have same configuration
            // regarding colors, normals and indices.
            if (!mesh->vertices.empty())
            {
            ...
            // Copy the mesh content into the combined mesh. This is done in
            // triplets for readability only, as one loop iteration will then
            // copy one triangle.
            for (size_t i = 0; i < mesh->vertices.size();
                 i += 3, new_index += 3)
            {
    ...
    /**
     * Get a connected mesh by merging close vertices and removing triangles
     * with zero surface area. If you only would like to connect vertices, make
     * sure that the proximity threhsold <<< voxel size. If you would like to
     * simplify the mesh, chose a threshold greater or near the voxel size until
     * you reached the level of simpliciation desired.
     */
    void getConnectedMesh(
        Mesh       *connected_mesh,
        const float approximate_vertex_proximity_threshold = 1e-10) const
    {
        BlockIndexList mesh_indices;
        getAllAllocatedMeshes(&mesh_indices);
