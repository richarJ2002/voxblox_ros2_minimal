 ...
 */
template <typename VoxelType> class Interpolator
{
  public:
    ...
    bool getGradient(const Point &pos,
                     Point       *grad,
                     const bool   interpolate = false) const;

    bool getDistance(const Point &pos,
                     float       *distance,
                     bool         interpolate = false) const;

    bool getWeight(const Point &pos,
                   float       *weight,
                   bool         interpolate = false) const;

    bool getVoxel(const Point &pos,
                  VoxelType   *voxel,
                  bool         interpolate = false) const;
     ...
     */
    bool getAdaptiveDistanceAndGradient(const Point &pos,
                                        float       *distance,
                                        Point       *grad) const;

    /// Without interpolation.
    bool getNearestDistanceAndWeight(const Point &pos,
                                     float       *distance,
                                     float       *weight) const;

    bool setIndexes(const Point   &pos,
                    BlockIndex    *block_index,
                    InterpIndexes *voxel_indexes) const;

    bool getVoxelsAndQVector(const Point      &pos,
  ...
  private:
     ...
     */
    void getQVector(const Point  &voxel_pos,
                    const Point  &pos,
    ...
    bool getInterpDistance(const Point &pos, float *distance) const;

    bool getNearestDistance(const Point &pos, float *distance) const;

    bool getInterpWeight(const Point &pos, float *weight) const;

    bool getNearestWeight(const Point &pos, float *weight) const;
    ...
    static float interpMember(const InterpVector &q_vector,
                              const VoxelType   **voxels,
