...
class OccupancyIntegrator
{
  public:
        ...
        layer_(layer)
    {
        ...
        min_occupancy_log_ =
            logOddsFromProbability(config_.threshold_occupancy);
    }
    ...
    inline void updateOccupancyVoxel(bool occupied, OccupancyVoxel *occ_voxel)
    {
        ...
        occ_voxel->probability_log =
            std::min(std::max(occ_voxel->probability_log + log_odds_update,
                              clamp_min_log_),
                     clamp_max_log_);
    }
                             ...
                             const Pointcloud     &points_C)
    {
        ...
        for (size_t pt_idx = 0; pt_idx < points_C.size(); ++pt_idx)
        {
            ...
            float                      ray_distance = (point_G - origin).norm();
            ...
            else if (ray_distance > config_.max_ray_length_m)
            {
                ...
                end_scaled = (origin + config_.max_ray_length_m * unit_ray) *
                             voxel_size_inv_;

                if (free_cells.find(getGridIndexFromPoint<GlobalIndex>(
                        end_scaled)) == free_cells.end())
                {
        ...
        for (const GlobalIndex &global_voxel_idx : free_cells)
        {
            ...
            VoxelIndex local_voxel_idx =
                getLocalFromGlobalVoxelIndex(global_voxel_idx,
                                             voxels_per_side_);
