...
static const float kUnitCubeDiagonalLength = std::sqrt(3.0);
                         ...
                         Layer<VoxelType>       *layer_out)
{
    ...
    {
        const Block<VoxelType> &input_block =
             ...
             ++input_linear_voxel_idx)
        {
            ...
            const Point voxel_center =
                T_out_in * input_block.computeCoordinatesFromLinearIndex(
                               input_linear_voxel_idx);
            ...
            typename Block<VoxelType>::Ptr output_block =
                layer_out->allocateBlockPtrByIndex(
                    getBlockIndexFromGlobalVoxelIndex(
                        global_output_voxel_idx,
                        layer_out->voxels_per_side_inv()));
            ...
            VoxelType &output_voxel = output_block->getVoxelByVoxelIndex(
                getLocalFromGlobalVoxelIndex(global_output_voxel_idx,
                                             layer_out->voxels_per_side()));
                    ...
                    Layer<VoxelType>       *layer_out)
{
    ...
    for (const BlockIndex &block_idx : block_idx_list_in)
    {
        ...
        float offset = kUnitCubeDiagonalLength * layer_in.block_size() * 0.5;
        ...
        *aligned_layers_and_error_layers = nullptr)
{
    ...
    for (size_t i = 0u; i < transforms_A_B.size(); ++i)
    {
        ...
        typename Layer<VoxelType>::Ptr aligned_layer_B(
            new Layer<VoxelType>(layer_B.voxel_size(),
                                 layer_B.voxels_per_side()));

        Layer<VoxelType> *error_layer = nullptr;
        if (aligned_layers_and_error_layers != nullptr)
        {
            if (aligned_layers_and_error_layers->size() !=
                transforms_A_B.size())
            {
            ...
            (*aligned_layers_and_error_layers)[i].second =
                typename voxblox::Layer<VoxelType>::Ptr(
                    new voxblox::Layer<VoxelType>(layer_A.voxel_size(),
                                                  layer_A.voxels_per_side()));
        ...
        *aligned_layers_and_error_layers = nullptr)
{
    ...
    evaluateLayerRmseAtPoses(voxel_evaluation_mode,
                             layer_A,
                             layer_B,
