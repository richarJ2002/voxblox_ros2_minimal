...
class TsdfIntegratorBase
{
  public:
     ...
     * truncation distance. Used when we are given a minimum distance to a
     * point, rather then exact distance. This is useful for clearing out free
     * space.
     */
  ...
  protected:
    /// Thread safe.
    inline bool isPointValid(const Point &point_C,
                             const bool   freespace_point,
                             bool        *is_clearing) const
    {
        DCHECK(is_clearing != nullptr);
        const float ray_distance = point_C.norm();
     ...
     * returned instead. Unlike the layer, accessing temp_block_map_ is
     ...
     */
    TsdfVoxel *
        allocateStorageAndGetVoxelPtr(const GlobalIndex     &global_voxel_idx,
                                      Block<TsdfVoxel>::Ptr *last_block,
                                      BlockIndex            *last_block_idx);

    /**
     * Merges temporarily stored blocks into the main layer. NOT thread safe,
     * see allocateStorageAndGetVoxelPtr for more details.
     */
    ...
    float computeDistance(const Point &origin,
                          const Point &point_G,
...
{
  public:
    static TsdfIntegratorBase::Ptr
        create(const std::string                &integrator_type_name,
               const TsdfIntegratorBase::Config &config,
               Layer<TsdfVoxel>                 *layer);
    static TsdfIntegratorBase::Ptr
        create(const TsdfIntegratorType          integrator_type,
               const TsdfIntegratorBase::Config &config,
               Layer<TsdfVoxel>                 *layer);
};
...
class FastTsdfIntegrator : public TsdfIntegratorBase
{
  ...
  private:
     ...
     * outlined in approx_hash_array.h, but in brief they are thread safe and
     * very fast, but have a small chance of returning false positives and false
