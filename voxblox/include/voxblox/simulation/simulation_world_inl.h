...
void SimulationWorld::generateSdfFromWorld(float             max_dist,
                                           Layer<VoxelType> *layer) const
{
    ...
    float half_block_size = block_size / 2.0;

    BlockIndexList blocks;
    for (float x = min_bound_.x() - half_block_size;
         x <= max_bound_.x() + half_block_size;
         x += block_size)
    {
        for (float y = min_bound_.y() - half_block_size;
             y <= max_bound_.y() + half_block_size;
             y += block_size)
        {
            for (float z = min_bound_.z() - half_block_size;
                 z <= max_bound_.z() + half_block_size;
                 z += block_size)
            {
                blocks.push_back(
    ...
    for (const BlockIndex &block_index : blocks)
    {
        ...
        for (size_t i = 0; i < block->num_voxels(); ++i)
        {
            ...
            if (!(coords.x() >= min_bound_.x() &&
                  coords.x() <= max_bound_.x() &&
                  coords.y() >= min_bound_.y() &&
                  coords.y() <= max_bound_.y() &&
...
template <>
inline void SimulationWorld::setVoxel(float        dist,
                                      const Color &color,
