...
class SdfIntegratorsTest : public ::testing::TestWithParam<float>
{
  public:
    SdfIntegratorsTest() :
    ...
    virtual void SetUp()
    {
        ...
        float cylinder_radius = 2;
        float cylinder_height = 4;
        world_.addObject(std::unique_ptr<Object>(new Cylinder(cylinder_center,
                                                              cylinder_radius,
                                                              cylinder_height,
                                                              Color::Red())));
        ...
        float angle_increment = max_angle / num_poses;
        for (float angle = 0.0; angle < max_angle; angle += angle_increment)
        {
            ...
            // Face the desired yaw and pitch forward a bit to get some of the
            // floor.
            Quaternion rotation =
                Quaternion(Eigen::AngleAxis<float>(-0.1, Point::UnitY())) *
                Eigen::AngleAxis<float>(desired_yaw, Point::UnitZ());
...
TEST_P(SdfIntegratorsTest, TsdfIntegrators)
{
    ...
    for (size_t i = 0; i < poses_.size(); i++)
    {
        ...
        world_.getPointcloudFromTransform(poses_[i],
    ...
    utils::evaluateLayersRmse(*tsdf_gt_,
                              simple_layer,
    ...
    constexpr float kfloatToleranceHigh = 1e-4;
    ...
    EXPECT_NEAR(simple_result.num_overlapping_voxels,
                merged_result.num_overlapping_voxels,
                one_percent_of_voxels);
    EXPECT_NEAR(simple_result.num_overlapping_voxels,
                fast_result.num_overlapping_voxels,
                one_percent_of_voxels);

    // Make sure they're all reasonable.
    EXPECT_NEAR(simple_result.min_error, 0.0, kfloatToleranceHigh);
    EXPECT_NEAR(merged_result.min_error, 0.0, kfloatToleranceHigh);
    EXPECT_NEAR(fast_result.min_error, 0.0, kfloatToleranceHigh);
...
TEST_P(SdfIntegratorsTest, EsdfIntegrators)
{
    ...
    EsdfIntegrator incremental_integrator(esdf_config,
                                          &tsdf_layer,
                                          &incremental_layer);
    EsdfIntegrator batch_integrator(esdf_config, &tsdf_layer, &batch_layer);
    esdf_config.full_euclidean_distance = true;
    EsdfIntegrator batch_full_euclidean_integrator(esdf_config,
                                                   &tsdf_layer,
    ...
    utils::evaluateLayersRmse(*esdf_gt_,
                              incremental_layer,
                              utils::VoxelEvaluationMode::kEvaluateAllVoxels,
                              &incremental_result);
    utils::evaluateLayersRmse(*esdf_gt_,
    ...
    // Figure out some metrics to compare against, based on voxel size.
    constexpr float kfloatToleranceHigh = 1e-4;
    constexpr float kKindaSimilar       = 1e-2;
    constexpr float kCloseEnough        = 1.0;

    // Make sure they're all reasonable.
    EXPECT_NEAR(incremental_result.min_error, 0.0, kfloatToleranceHigh);
    EXPECT_NEAR(batch_result.min_error, 0.0, kfloatToleranceHigh);
    EXPECT_NEAR(batch_full_euclidean_result.min_error,
                0.0,
                kfloatToleranceHigh);
    ...
    EXPECT_LT(batch_full_euclidean_result.rmse,
              esdf_max_distance_ * voxel_size_);
    ...
    EXPECT_NEAR(incremental_result.max_error,
                batch_result.max_error,
                kCloseEnough);

    // Output for debugging.
    io::SaveLayer(tsdf_layer, "esdf_euclidean_test.voxblox", true);
    io::SaveLayer(batch_full_euclidean_layer,
                  "esdf_euclidean_test.voxblox",
                  false);
...
INSTANTIATE_TEST_CASE_P(VoxelSizes,
                        SdfIntegratorsTest,
