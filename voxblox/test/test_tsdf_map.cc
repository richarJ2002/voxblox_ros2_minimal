...
TEST_F(TsdfMapTest, IndexLookups)
{
    ...
    EXPECT_TRUE(EIGEN_MATRIX_EQUAL(
        map_->getTsdfLayerPtr()->computeBlockIndexFromCoordinates(
            point_in_0_0_0),
        AnyIndex(0, 0, 0)));
    EXPECT_TRUE(EIGEN_MATRIX_NEAR(block_0_0_0->origin(),
                                  Point(0.0, 0.0, 0.0),
                                  kEpsilon));
    EXPECT_TRUE(
    ...
    EXPECT_TRUE(EIGEN_MATRIX_NEAR(block_0_0_0_v2->origin(),
                                  Point(0.0, 0.0, 0.0),
                                  kEpsilon));
    EXPECT_TRUE(
    ...
    Point                 point_in_1_1_1_v2(block_size_,
                            block_size_ + config_.tsdf_voxel_size,
                            block_size_);
    ...
    EXPECT_TRUE(EIGEN_MATRIX_EQUAL(
        ...
        AnyIndex(-1, -1, -1)));
    EXPECT_TRUE(
        EIGEN_MATRIX_NEAR(block_negative_1_1_1->origin(),
                          Point(-block_size_, -block_size_, -block_size_),
                          kEpsilon));
    ...
    // BLOCK -1 -1 -1 (coordinate within block)
    Point                 point_in_neg_1_1_1_v2(-block_size_,
                                -block_size_ + config_.tsdf_voxel_size,
                                -block_size_);
    ...
    EXPECT_TRUE(
        EIGEN_MATRIX_NEAR(block_negative_1_1_1_v2->origin(),
                          Point(-block_size_, -block_size_, -block_size_),
                          kEpsilon));
    ...
    // Test in Block 0 0 0
    {
        ...
        EXPECT_NE(block_0_0_0->getVoxelPtrByCoordinates(point_in_0_0_0),
                  nullptr);
        ...
        VoxelIndex voxel_index =
            block_0_0_0->computeTruncatedVoxelIndexFromCoordinates(
                point_in_0_0_0);
    ...
    }
    {
        ...
        VoxelIndex voxel_index =
            block_0_0_0->computeTruncatedVoxelIndexFromCoordinates(
                point_in_0_0_0);
        EXPECT_TRUE(
            EIGEN_MATRIX_EQUAL(voxel_index,
    ...
    // Test in Block -1 -1 -1
    {
        ...
        EXPECT_TRUE(EIGEN_MATRIX_EQUAL(
            voxel_index,
            block_negative_1_1_1->computeVoxelIndexFromLinearIndex(
                linear_index)));
        EXPECT_TRUE(EIGEN_MATRIX_NEAR(
            point_in_neg_1_1_1,
            block_negative_1_1_1->computeCoordinatesFromLinearIndex(
                linear_index),
            config_.tsdf_voxel_size));
        EXPECT_TRUE(EIGEN_MATRIX_NEAR(
            point_in_neg_1_1_1,
    ...
    // Test in Block -1 -1 0
    {
        ...
        EXPECT_TRUE(EIGEN_MATRIX_EQUAL(
            ...
            AnyIndex(-1, -1, 0)));
        EXPECT_NE(
            block_neg_1_neg_1_0->getVoxelPtrByCoordinates(point_in_neg_1_1_0),
            nullptr);
        ...
        EXPECT_TRUE(EIGEN_MATRIX_EQUAL(
            voxel_index,
            block_neg_1_neg_1_0->computeVoxelIndexFromLinearIndex(
                linear_index)));
        EXPECT_TRUE(EIGEN_MATRIX_NEAR(
            point_in_neg_1_1_0,
            block_neg_1_neg_1_0->computeCoordinatesFromLinearIndex(
                linear_index),
            config_.tsdf_voxel_size));
        EXPECT_TRUE(EIGEN_MATRIX_NEAR(
            point_in_neg_1_1_0,
...
TEST_F(TsdfMapTest, ComputeBlockIndexFromOriginFromBlockIndexTest)
{
    ...
    constexpr float  kBlockSize           = 0.32;
    constexpr float  kBlockSizeInv        = 1.0 / kBlockSize;
    ...
    for (int32_t x = -half_index_range; x <= half_index_range; ++x)
    {
        for (int32_t y = -half_index_range; y <= half_index_range; ++y)
        {
            for (int32_t z = -half_index_range; z <= half_index_range; ++z)
            {
                ...
                Point      block_origin =
                    getOriginPointFromGridIndex(block_idx, kBlockSize);
                    ...
                    (block_idx.z() != block_idx_from_origin.z()))
                {
                    ...
                    std::cout << "Block idx: " << block_idx.transpose()
                              << std::endl;
                    std::cout << "Block origin: " << block_origin.transpose()
                              << std::endl;
                    std::cout << "Block idx after: "
                              << block_idx_from_origin.transpose() << std::endl
                              << std::endl;
                    ASSERT_TRUE(false);
                }
            }
