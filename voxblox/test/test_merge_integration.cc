                           ...
                           public voxblox::test::LayerTest<VoxelType>
{
  protected:
    ...
    void initializeSimulatedWorlds()
    {
        ...
        world_B_compare_.reset(
            new Layer<VoxelType>(voxel_size_, voxels_per_side_));
    ...
    T_B_A <<  0.959292,  0.233991, 0.158138, -0.0627464,
        ...
        Point c1_A;
        c1_A << 0.1, 0., 0.;
        constexpr float sphere_1_radius_m = 0.05;

        // Define sphere 2.
        Point c2_A;
        c2_A << -0.2, 0, 0.2;
        constexpr float sphere_2_radius_m = 0.03;

        // Define sphere 3.
        Point c3_A;
        c3_A << 0., 0.1, -0.2;
        constexpr float sphere_3_radius_m = 0.075;
        ...
        simulation.addObject(
            std::unique_ptr<Object>(new Sphere(T_B_A_.transform(c1_A),
                                               sphere_1_radius_m,
                                               Color(0u, 255u, 0u))));
        simulation.addObject(
            std::unique_ptr<Object>(new Sphere(T_B_A_.transform(c2_A),
                                               sphere_2_radius_m,
                                               Color(0u, 255u, 0u))));
        simulation.addObject(
            std::unique_ptr<Object>(new Sphere(T_B_A_.transform(c3_A),
                                               sphere_3_radius_m,
                                               Color(0u, 255u, 0u))));
        simulation.generateSdfFromWorld(max_distance_world, world_B_.get());
        simulation.generateSdfFromWorld(max_distance_world,
                                        world_B_compare_.get());
    }
...
TEST_F(TsdfLayerMergeToolTest, MergeTwoTsdfLayers)
{
    ...
    io::outputLayerAsPly(*world_A_,
                         kWorldAPlyFile,
                         io::PlyOutputTypes::kSdfIsosurfaceConnected);
    const std::string kWorldASdfPlyFile =
        kFolderPrefix + "world_A.tsdf_sdf.ply";
    io::outputLayerAsPly(*world_A_,
                         kWorldASdfPlyFile,
                         io::PlyOutputTypes::kSdfColoredDistanceField,
                         max_sdf_value_to_visualize,
                         max_sdf_value_to_visualize);

    const std::string kWorldBPlyFile = kFolderPrefix + "world_B.tsdf.ply";
    LOG(INFO) << "Writing world B to : " << kWorldBPlyFile;
    io::outputLayerAsPly(*world_B_,
                         kWorldBPlyFile,
                         io::PlyOutputTypes::kSdfIsosurfaceConnected);
    const std::string kWorldBSdfPlyFile =
        kFolderPrefix + "world_B.tsdf_sdf.ply";
    io::outputLayerAsPly(*world_B_,
                         kWorldBSdfPlyFile,
                         io::PlyOutputTypes::kSdfColoredDistanceField,
                         max_sdf_value_to_visualize,
    ...
    utils::evaluateLayersRmse(*world_B_compare_,
                              *world_B_,
                              utils::VoxelEvaluationMode::kEvaluateAllVoxels,
                              &result);
    constexpr float kfloatToleranceLow = 5e-5;
    EXPECT_NEAR(result.rmse, 6e-5, kfloatToleranceLow);

    constexpr float kfloatToleranceHigh = 1e-4;
    EXPECT_NEAR(result.min_error, 0.0, kfloatToleranceHigh);
    EXPECT_NEAR(result.max_error, 0.002, kfloatToleranceHigh);
