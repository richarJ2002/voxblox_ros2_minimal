...
int main(int argc, char **argv)
{
    ...
    const float       min_weight         = std::stof(argv[3]);
    const float       min_distance_m     = std::stof(argv[4]);
    const float       max_distance_m     = std::stof(argv[5]);
    const float       default_distance_m = std::stof(argv[6]);
