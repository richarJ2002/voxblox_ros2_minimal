...
{
    float distance_fp;
    bool  success = interpolator_.getDistance(position.cast<float>(),
                                             &distance_fp,
                                             interpolate);
    if (success)
    {
...
bool EsdfMap::getDistanceAndGradientAtPosition(const Eigen::Vector3d &position,
                                               double                *distance,
                                               Eigen::Vector3d *gradient) const
{
    constexpr bool interpolate = true;
    return getDistanceAndGradientAtPosition(position,
                                            interpolate,
                                            distance,
                                            gradient);
}
                                               ...
                                               Eigen::Vector3d *gradient) const
{
    ...
    {
        success =
            interpolator_.getAdaptiveDistanceAndGradient(position.cast<float>(),
                                                         &distance_fp,
                                                         &gradient_fp);
    }
    else
    {
        success = interpolator_.getDistance(position.cast<float>(),
                                            &distance_fp,
                                            interpolate);
        success &= interpolator_.getGradient(position.cast<float>(),
                                             &gradient_fp,
                                             interpolate);
    }
...
bool EsdfMap::isObserved(const Eigen::Vector3d &position) const
{
    ...
    Block<EsdfVoxel>::Ptr block_ptr =
        esdf_layer_->getBlockPtrByCoordinates(position.cast<float>());
    if (block_ptr)
    {
        const EsdfVoxel &voxel =
            block_ptr->getVoxelByCoordinates(position.cast<float>());
    ...
    Eigen::Ref<Eigen::VectorXi>                                observed) const
{
    if (distances.size() < positions.cols())
    {
        throw std::runtime_error(
            "Distances array smaller than number of queries");
    }

    if (observed.size() < positions.cols())
    {
        throw std::runtime_error(
            "Observed array smaller than number of queries");
    }
    ...
    Eigen::Ref<Eigen::VectorXi>                                observed) const
{
    ...
    {
        throw std::runtime_error(
            "Gradients matrix smaller than number of queries");
    }
