...
TsdfIntegratorBase::Ptr
    TsdfIntegratorFactory::create(const std::string &integrator_type_name,
                                  ...
                                  Layer<TsdfVoxel>                 *layer)
{
         ...
         kTsdfIntegratorTypeNames)
    {
        ...
        {
            return create(static_cast<TsdfIntegratorType>(integrator_type),
                          config,
                          layer);
        }
        ++integrator_type;
...
TsdfIntegratorBase::Ptr
    TsdfIntegratorFactory::create(const TsdfIntegratorType integrator_type,
                                  const TsdfIntegratorBase::Config &config,
                                  Layer<TsdfVoxel>                 *layer)
{
    CHECK_NOTNULL(layer);
    ...
    BlockIndex            *last_block_idx)
{
    ...
    const BlockIndex block_idx =
        getBlockIndexFromGlobalVoxelIndex(global_voxel_idx,
    ...
    {
        // To allow temp_block_map_ to grow we can only let one thread in at
        // once
                                         ...
                                         TsdfVoxel         *tsdf_voxel)
{
    ...
    // already computed.
    const float dropoff_epsilon = voxel_size_;
    if (config_.use_weight_dropoff && sdf < -dropoff_epsilon)
    {
        updated_weight =
            weight * (config_.default_truncation_distance + sdf) /
...
float TsdfIntegratorBase::getVoxelWeight(const Point &point_C) const
{
    ...
    const float dist_z = std::abs(point_C.z());
    if (dist_z > kEpsilon)
    {
                                               ...
                                               const bool freespace_points)
{
    ...
    for (size_t i = 0; i < config_.integrator_threads; ++i)
    {
        integration_threads.emplace_back(
            &SimpleTsdfIntegrator::integrateFunction,
            this,
            T_G_C,
            points_C,
            colors,
    ...
    const LongIndexHashMapType<AlignedVector<size_t>>::type &voxel_map)
{
    ...
    for (const size_t pt_idx : kv.second)
    {
        ...
        merged_color = Color::blendTwoColors(merged_color,
                                             merged_weight,
                                             color,
                                             point_weight);
        merged_weight += point_weight;
    ...
    const LongIndexHashMapType<AlignedVector<size_t>>::type &clear_map)
{
    ...
    else
    {
        ...
        {
            integration_threads.emplace_back(
                &MergedTsdfIntegrator::integrateVoxels,
                this,
                T_G_C,
                points_C,
                colors,
                                           ...
                                           ThreadSafeIndex *index_getter)
{
                ...
                .count() < config_.max_integration_time_s * 1000000))
    {
        ...
        while (ray_caster.nextRayIndex(&global_voxel_idx))
        {
            // Check if the current voxel has been seen by any ray cast this
            // scan. If it has increment the consecutive_ray_collisions counter,
            // otherwise reset it. If the counter reaches a threshold we stop
            // casting as the ray is deemed to be contributing too little new
            // information.
