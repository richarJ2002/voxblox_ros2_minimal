    ...
    ThreadSafeIndex(points_C.size())
{
    ...
    std::sort(indices_and_squared_norms_.begin(),
              indices_and_squared_norms_.end(),
...
RayCaster::RayCaster(const Point &origin,
                     const Point &point_G,
                     const bool   is_clearing_ray,
                     const bool   voxel_carving_enabled,
                     const float  max_ray_length_m,
                     ...
                     const bool   cast_from_origin)
{
    ...
    if (is_clearing_ray)
    {
        float ray_length = (point_G - origin).norm();
        ray_length       = std::min(
            std::max(ray_length - truncation_distance, static_cast<float>(0.0)),
            max_ray_length_m);
                               ...
                               const Point &end_scaled)
{
    ...
    ray_step_signs_ = AnyIndex(signum(ray_scaled.x()),
                               signum(ray_scaled.y()),
                               signum(ray_scaled.z()));
    ...
    const Point start_scaled_shifted = start_scaled - curr_index_.cast<float>();

    Ray distance_to_boundaries(corrected_step.cast<float>() -
