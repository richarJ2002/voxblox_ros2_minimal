...
void EsdfIntegrator::addNewRobotPosition(const Point &position)
{
    ...
    for (const std::pair<BlockIndex, VoxelIndexList> &kv : block_voxel_list_occ)
    {
        ...
        for (const VoxelIndex &voxel_index : kv.second)
        {
            ...
            {
                GlobalIndex global_index =
                    getGlobalVoxelIndexFromBlockAndVoxelIndex(kv.first,
                                                              voxel_index,
                                                              voxels_per_side_);
...
void EsdfIntegrator::updateFromTsdfLayerBatch()
{
    ...
    tsdf_blocks.insert(tsdf_blocks.end(),
                       updated_blocks_.begin(),
                       updated_blocks_.end());
...
{
    /* Init list of tsdf blocks to update */
    BlockIndexList tsdf_blocks;

    /* Get all the updated blocks and store in tsdf_blocks */
    tsdf_layer_->getAllUpdatedBlocks(Update::kEsdf, &tsdf_blocks);

    /* Update `tsdf_blocks` with blocks from `updated_blocks_` */
    tsdf_blocks.insert(tsdf_blocks.end(),
                       updated_blocks_.begin(),
                       updated_blocks_.end());

    /* Clear the `updated_blocks_` so no duplicates are put in the tsdf */
    updated_blocks_.clear();

    /*!
     * This variable is only here because someone couldn't be bothered to write
     * an actual doxygen comment for the method `updateFromTsdfBlocks`. Hence,
     * its soul purpose is to make it clear that we are making the kIncrimental
     * option of the updateFromTsdf true.
     *
     * TODO:        Remove variable, write an actual doxygen comment for method
     *              and change method call to:
     *              updateFromTsdfBlocks(tsdf_blocks, true);
     */
    const bool kIncremental = true;

    /* Update the esdf based on the updates from the changes to the tsdf */
    updateFromTsdfBlocks(tsdf_blocks, true);

    /* Clear the tsdf memory. (This is to optimze on memory) */
                                          ...
                                          bool                  kIncremental)
{
    ...
    for (const BlockIndex &block_index : tsdf_blocks)
    {
        /* Find layer of tsdf based on `block_index` */
        Block<TsdfVoxel>::ConstPtr tsdf_block =
            tsdf_layer_->getBlockPtrByIndex(block_index);

        /* If block is not valid, skip onto next block */
        if (!tsdf_block)
        {
            continue;
        }
        ...
        /*!
         * Allocate the same block in the ESDF layer. Block indices are the same
         * across all layers.
         */
        Block<EsdfVoxel>::Ptr esdf_block =
            esdf_layer_->allocateBlockPtrByIndex(block_index);

        /* Set flag to indicate that the block is being updated */
        esdf_block->set_updated(true);

        /* Find the number of voxels per block */
        const size_t num_voxels_per_block = tsdf_block->num_voxels();

        /* For every num of voxels analyse. */
             ...
             ++lin_index)
        {
            ...
            {
                if (!kIncremental && config_.add_occupied_crust)
                {
            ...
            GlobalIndex global_index =
                getGlobalVoxelIndexFromBlockAndVoxelIndex(block_index,
                                                          voxel_index,
                                                          voxels_per_side_);
            ...
            if (!esdf_voxel.observed || esdf_voxel.hallucinated)
            {
                ...
                else
                {
                    ...
                    esdf_voxel.distance = signum(tsdf_voxel.distance) *
            ...
            else
            {
                ...
                // (1) lower: esdf or tsdf is fixed, and tsdf is closer to
                // surface than it used to be. (2) raise: esdf or tsdf is fixed,
                // and tsdf is further from surface than it used to be. (3) sign
                // flip: tsdf and esdf have different signs, otherwise the lower
                // and raise rules apply as above.
                ...
                // Otherwise we just don't care. Not fixed voxels that match the
                // right sign can be whatever value that they want to be.
            }
...
void EsdfIntegrator::processRaiseSet()
{
    ...
    while (!raise_.empty())
    {
        ...
        for (unsigned int idx = 0u; idx < neighbor_indices.cols(); ++idx)
        {
            ...
            {
                // If it's not in the queue, then add it to open so it can
                // update our weights back.
    ...
    while (!open_.empty())
    {
        ...
        for (unsigned int idx = 0u; idx < neighbor_indices.cols(); ++idx)
        {
            ...
            if (config_.full_euclidean_distance)
            {
                ...
                distance   = voxel_size_ * (new_parent.cast<float>().norm() -
                                          voxel->parent.cast<float>().norm());
            ...
            {
                const float potential_distance =
...
bool EsdfIntegrator::updateVoxelFromNeighbors(const GlobalIndex &global_index)
{
    ...
    {
        const GlobalIndex &neighbor_index = neighbor_indices.col(idx);
        const float        distance       = Neighborhood<>::kDistances[idx];
        ...
        if (signum(neighbor_voxel->distance) == signum(voxel->distance))
        {
            ...
            {
                voxel->distance = neighbor_voxel->distance +
                                  signum(voxel->distance) * distance;
