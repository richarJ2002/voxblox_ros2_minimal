    ...
    min_time_between_msgs_(rclcpp::Duration::from_seconds(1))
{
    ...
    if (enable_icp_)
    {
        ...
        {
            node_ptr_->declare_parameter("icp_corrected_frame",
    ...
    node_ptr_->get_parameter("update_mesh_every_n_sec",
                             update_mesh_every_n_sec);
    ...
    node_ptr_->get_parameter("publish_map_every_n_sec",
                             publish_map_every_n_sec);
    ...
    surface_pointcloud_pub_ =
        node_ptr_->create_publisher<sensor_msgs::msg::PointCloud2>(
            generate_private_name(node_ptr_, "surface_pointcloud"),
            1);

    tsdf_pointcloud_pub_ =
        node_ptr_->create_publisher<sensor_msgs::msg::PointCloud2>(
            generate_private_name(node_ptr_, "tsdf_pointcloud"),
            1);

    occupancy_marker_pub_ =
        node_ptr_->create_publisher<visualization_msgs::msg::MarkerArray>(
            generate_private_name(node_ptr_, "occupied_nodes"),
            1);

    tsdf_slice_pub_ =
        node_ptr_->create_publisher<sensor_msgs::msg::PointCloud2>(
            generate_private_name(node_ptr_, "tsdf_slice"),
            1);

    mesh_pub_ = node_ptr_->create_publisher<voxblox_msgs::msg::Mesh>(
        generate_private_name(node_ptr_, "mesh"),
        1);

    tsdf_map_pub_ = node_ptr_->create_publisher<voxblox_msgs::msg::Layer>(
        generate_private_name(node_ptr_, "tsdf_map_out"),
        1);
    ...
    if (enable_icp_)
    {
        icp_transform_pub_ =
            node_ptr_->create_publisher<geometry_msgs::msg::TransformStamped>(
                generate_private_name(node_ptr_, "icp_transform"),
                1);
    }
    ...
    pointcloud_sub_ =
        node_ptr_->create_subscription<sensor_msgs::msg::PointCloud2>(
            ...
            pointcloud_queue_size_,
            std::bind(&TsdfServer::insertPointcloud,
                      this,
                      std::placeholders::_1));

    tsdf_map_sub_ = node_ptr_->create_subscription<voxblox_msgs::msg::Layer>(
        generate_private_name(node_ptr_, "tsdf_map_in"),
        1,
    ...
    if (use_freespace_pointcloud_)
    {
        freespace_pointcloud_sub_ =
            node_ptr_->create_subscription<sensor_msgs::msg::PointCloud2>(
                generate_private_name(node_ptr_, "freespace_pointcloud"),
                pointcloud_queue_size_,
                std::bind(&TsdfServer::insertFreespacePointcloud,
                          this,
                          std::placeholders::_1));
    }
    ...
    if (method.compare("simple") == 0)
    {
        tsdf_integrator_.reset(
            new SimpleTsdfIntegrator(integrator_config,
                                     tsdf_map_->getTsdfLayerPtr()));
    }
    else if (method.compare("merged") == 0)
    {
        tsdf_integrator_.reset(
            new MergedTsdfIntegrator(integrator_config,
                                     tsdf_map_->getTsdfLayerPtr()));
    }
    else if (method.compare("fast") == 0)
    {
        tsdf_integrator_.reset(
            new FastTsdfIntegrator(integrator_config,
                                   tsdf_map_->getTsdfLayerPtr()));
    }
    else
    {
    ...
    mesh_integrator_.reset(
        new MeshIntegrator<TsdfVoxel>(mesh_config,
                                      tsdf_map_->getTsdfLayerPtr(),
                                      mesh_layer_.get()));
    ...
    generate_mesh_srv_ = node_ptr_->create_service<std_srvs::srv::Empty>(
        generate_private_name(node_ptr_, "generate_mesh"),
        std::bind(&TsdfServer::generateMeshCallback,
                  this,
                  std::placeholders::_1,
                  std::placeholders::_2));

    clear_map_srv_ = node_ptr_->create_service<std_srvs::srv::Empty>(
        generate_private_name(node_ptr_, "clear_map"),
        std::bind(&TsdfServer::clearMapCallback,
                  this,
                  std::placeholders::_1,
                  std::placeholders::_2));

    save_map_srv_ = node_ptr_->create_service<voxblox_msgs::srv::FilePath>(
        generate_private_name(node_ptr_, "save_map"),
        std::bind(&TsdfServer::saveMapCallback,
                  this,
                  std::placeholders::_1,
                  std::placeholders::_2));

    load_map_srv_ = node_ptr_->create_service<voxblox_msgs::srv::FilePath>(
        generate_private_name(node_ptr_, "load_map"),
        std::bind(&TsdfServer::loadMapCallback,
                  this,
                  std::placeholders::_1,
                  std::placeholders::_2));

    publish_pointclouds_srv_ = node_ptr_->create_service<std_srvs::srv::Empty>(
        generate_private_name(node_ptr_, "publish_pointclouds"),
        std::bind(&TsdfServer::publishPointcloudsCallback,
                  this,
                  std::placeholders::_1,
                  std::placeholders::_2));

    publish_tsdf_map_srv_ = node_ptr_->create_service<std_srvs::srv::Empty>(
        generate_private_name(node_ptr_, "publish_map"),
        std::bind(&TsdfServer::publishTsdfMapCallback,
                  this,
                  std::placeholders::_1,
                  std::placeholders::_2));
...
void TsdfServer::getServerConfigFromRosParam(rclcpp::Node *node_ptr)
{
    ...
    {
        node_ptr->declare_parameter("publish_pointclouds",
    ...
    {
        node_ptr->declare_parameter("intensity_colormap",
                                    std::string("rainbow"));
    }
    if (!node_ptr->has_parameter("intensity_max_value"))
    {
        node_ptr->declare_parameter("intensity_max_value",
                                    kDefaultMaxIntensity);
    }
    ...
    const bool                                      is_freespace_pointcloud)
{
    ...
    if (enable_icp_)
    {
        ...
        const size_t          num_icp_updates =
            icp_->runICP(tsdf_map_->getTsdfLayer(),
                         points_C,
                         icp_corrected_transform_ * T_G_C,
                         &T_G_C_refined);
        if (verbose_)
        {
        ...
        {
            // its already removed internally but small floating point errors
            // can build up if accumulating transforms
    ...
    if (verbose_)
    {
        // ROS_INFO("Integrating a pointcloud with %lu points.",
        // points_C.size());
        RCLCPP_INFO(node_ptr_->get_logger(),
    ...
    integratePointcloud(T_G_C_refined,
                        points_C,
                        colors,
    ...
    const sensor_msgs::msg::PointCloud2::SharedPtr pointcloud_msg_in)
{
        ...
        getNextPointcloudFromQueue(&pointcloud_queue_, &pointcloud_msg, &T_G_C))
    {
        ...
        processPointCloudMessageAndInsert(pointcloud_msg,
                                          T_G_C,
                                          is_freespace_pointcloud);
        processed_any = true;
    }
                                     ...
                                     const bool is_freespace_pointcloud)
{
    ...
    tsdf_integrator_->integratePointCloud(T_G_C,
                                          ptcloud_C,
                                          colors,
                                          is_freespace_pointcloud);
}
...
void TsdfServer::publishAllUpdatedTsdfVoxels()
{
    ...
    createDistancePointcloudFromTsdfLayer(tsdf_map_->getTsdfLayer(),
                                          &pointcloud);
...
void TsdfServer::publishSlices()
{
    ...
    createDistancePointcloudFromTsdfLayerSlice(tsdf_map_->getTsdfLayer(),
                                               2,
                                               slice_level_,
                                               &pointcloud);
...
void TsdfServer::publishMap(bool reset_remote_map)
{
    ...
    if (subscribers > 0)
    {
        if (num_subscribers_tsdf_map_ < subscribers)
        {
            ...
            // subscribes. A bit of overhead for other subscribers, but better
            // than inconsistent map states.
...
bool TsdfServer::generateMesh()
{
    ...
    if (!mesh_filename_.empty())
    {
        ...
        else
        {
            ...
            // mesh_filename_.c_str());
            RCLCPP_INFO(node_ptr_->get_logger(),
                        "Failed to output mesh as PLY: %s",
                        mesh_filename_.c_str());
        }
    }
...
bool TsdfServer::loadMap(const std::string &file_path)
{
    ...
    bool           success =
        io::LoadBlocksFromFile(file_path,
...
void TsdfServer::tsdfMapCallback(const voxblox_msgs::msg::Layer &layer_msg)
{
    ...
    bool success =
        deserializeMsgToLayer<TsdfVoxel>(layer_msg,
                                         tsdf_map_->getTsdfLayerPtr());
