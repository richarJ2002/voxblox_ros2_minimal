...
class VoxbloxEvaluator
{
  ...
  private:
    ...
    bool        visualize_;
    // Whether to recolor the voxels by error to the GT before generating a
    // mesh.
    ...
    frame_id_("world")
{
    ...
    if (node_ptr_->get_parameter("T_V_G", T_V_G_xml))
    {
        ...
        {
            node_ptr_->declare_parameter("invert_T_V_G",
    ...
    CHECK(node_ptr_->get_parameter("voxblox_file_path", voxblox_file_path))
        << "No file path provided for voxblox map! Set the "
           "\"voxblox_file_path\" "
           "param.";
...
void VoxbloxEvaluator::evaluate()
{
         ...
         ++it)
    {
        ...
        // voxel exists.
        if (!interpolator_->getNearestDistanceAndWeight(point,
                                                        &distance,
                                                        &weight))
        {
            unknown_voxels++;
