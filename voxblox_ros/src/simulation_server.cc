...
void SimulationServer::getServerConfigFromRosParam(rclcpp::Node *node_ptr)
{
    ...
    // nh_private.param("depth_camera_resolution_u",
    // depth_camera_resolution_[0],
    //                  depth_camera_resolution_[0]);
    // nh_private.param("depth_camera_resolution_v",
    ...
    world_(new SimulationWorld())
{
    ...
    esdf_integrator_.reset(new EsdfIntegrator(esdf_integrator_config_copy,
                                              tsdf_test_.get(),
                                              esdf_test_.get()));
    ...
    {
        occ_test_.reset(
            new Layer<OccupancyVoxel>(voxel_size_, voxels_per_side_));
        ...
        esdf_occ_integrator_.reset(new EsdfOccIntegrator(esdf_occ_config,
                                                         occ_test_.get(),
                                                         esdf_test_.get()));
    }
    ...
    tsdf_gt_pub_ =
        node_ptr_->create_publisher<sensor_msgs::msg::PointCloud2>("tsdf_gt",
                                                                   1);
    esdf_gt_pub_ =
        node_ptr_->create_publisher<sensor_msgs::msg::PointCloud2>("esdf_gt",
                                                                   1);
    tsdf_gt_mesh_pub_ =
        node_ptr_->create_publisher<visualization_msgs::msg::MarkerArray>(
            ...
            1);
    tsdf_test_pub_ =
        node_ptr_->create_publisher<sensor_msgs::msg::PointCloud2>("tsdf_test",
                                                                   1);
    esdf_test_pub_ =
        node_ptr_->create_publisher<sensor_msgs::msg::PointCloud2>("esdf_test",
                                                                   1);
    tsdf_test_mesh_pub_ =
        node_ptr_->create_publisher<visualization_msgs::msg::MarkerArray>(
            "tsdf_test_mesh",
...
bool SimulationServer::generatePlausibleViewpoint(float  min_distance,
                                                  Point *ray_origin,
                                                  Point *ray_direction) const
{
    ...
    float max_distance = min_distance * 2.0;
...
void SimulationServer::generateSDF()
{
    ...
    {
        if (!generatePlausibleViewpoint(min_dist_,
                                        &view_origin,
                                        &view_direction))
        {
            // ROS_WARN(
        ...
        Transformation T_G_C(
            view_origin,
            Eigen::Quaternion<float>::FromTwoVectors(Point(0.0, 0.0, 1.0),
                                                     view_direction));
...
void SimulationServer::evaluate()
{
    ...
    // ROS_INFO_STREAM("TSDF RMSE: " << tsdf_rmse << " ESDF RMSE: " <<
    // esdf_rmse);
...
void SimulationServer::visualize()
{
    ...
    createDistancePointcloudFromTsdfLayerSlice(*tsdf_gt_,
                                               2,
                                               visualization_slice_level_,
                                               &pointcloud);
    ...
    pointcloud.clear();
    createDistancePointcloudFromEsdfLayerSlice(*esdf_gt_,
                                               2,
                                               visualization_slice_level_,
                                               &pointcloud);
    ...
    pointcloud.clear();
    createDistancePointcloudFromTsdfLayerSlice(*tsdf_test_,
                                               2,
                                               visualization_slice_level_,
                                               &pointcloud);
    ...
    pointcloud.clear();
    createDistancePointcloudFromEsdfLayerSlice(*esdf_test_,
                                               2,
                                               visualization_slice_level_,
                                               &pointcloud);
    ...
    if (generate_mesh_)
    {
        ...
        MeshIntegrator<TsdfVoxel> mesh_integrator(mesh_config,
                                                  tsdf_gt_.get(),
                                                  mesh.get());
        ...
        MeshIntegrator<TsdfVoxel> mesh_integrator_test(mesh_config,
                                                       tsdf_test_.get(),
                                                       mesh_test.get());
