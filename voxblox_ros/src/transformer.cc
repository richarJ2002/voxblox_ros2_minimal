    ...
    timestamp_tolerance_ns_(1000000)
{
    ...
    {
        node_ptr_->declare_parameter<std::string>("sensor_frame",
                                                  sensor_frame_);
    }
    ...
    node_ptr_->get_parameter("timestamp_tolerance_sec",
                             timestamp_tolerance_sec);
    ...
    if (!use_tf_transforms_)
    {
        RCLCPP_WARN(node_ptr_->get_logger(),
                    "Using transform topic and static transforms from "
                    "parameters. This is "
        ...
        transform_sub_ptr_ =
            node_ptr_
                ->create_subscription<geometry_msgs::msg::TransformStamped>(
                    "transform",
                    rclcpp::QoS(40),
                    std::bind(&Transformer::transformCallback,
                              this,
                              std::placeholders::_1));
        ...
        if (node_ptr_->get_parameter("T_B_D", T_B_D_xml))
        {
            ...
            node_ptr_->declare_parameter("invert_T_B_D",
                                         invert_static_tranform);
        ...
        if (node_ptr_->get_parameter("T_B_C", T_B_C_xml))
        {
            ...
            /* nh_private_.param("invert_T_B_C", invert_static_tranform,
                              invert_static_tranform); */
            node_ptr_->declare_parameter("invert_T_B_C",
                                         invert_static_tranform);
                                    ...
                                    Transformation     *transform)
{
    ...
    if (!tf_buffer_->canTransform(to_frame,
                                  from_frame_modified,
                                  time_to_lookup,
    ...
    try
    {
        tf_transform = tf_buffer_->lookupTransform(to_frame,
                                                   from_frame_modified,
                                                   time_to_lookup);
    }
    ...
    { // NOLINT
        ...
        RCLCPP_ERROR(node_ptr_->get_logger(),
                     "Error getting TF transform from sensor data: %s",
                     ex.what());
        return false;
    }
