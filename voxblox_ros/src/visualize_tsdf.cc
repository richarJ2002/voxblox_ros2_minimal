...
{
  public:
    explicit SimpleTsdfVisualizer(rclcpp::Node *node_ptr) :
        node_ptr_(node_ptr),
        ...
        tsdf_voxel_ply_output_path_("")
    {
        ...
        // mesh_pub_ = nh_private_.advertise<voxblox_msgs::Mesh>("mesh", 1,
        // true);
        ...
        // mesh_pcl_mesh_pub_ =
        //     nh_private_.advertise<pcl_msgs::PolygonMesh>("mesh_pcl", 1,
        //     true);
        ...
        tsdf_pointcloud_pub_ =
            node_ptr_->create_publisher<sensor_msgs::msg::PointCloud2>(
                ...
                1);
        mesh_pub_ =
            node_ptr_->create_publisher<voxblox_msgs::msg::Mesh>("mesh", 1);
        mesh_pointcloud_pub_ =
            node_ptr_->create_publisher<sensor_msgs::msg::PointCloud2>(
                "mesh_as_pointcloud",
                1);
        mesh_pcl_mesh_pub_ =
            node_ptr_->create_publisher<pcl_msgs::msg::PolygonMesh>("mesh_pcl",
                                                                    1);

        // ROS_DEBUG_STREAM("\tRetreiving ROS parameters...");
        RCLCPP_INFO_STREAM(node_ptr_->get_logger(),
                           "\tRetreiving ROS parameters...");

        // nh_private_.param("tsdf_surface_distance_threshold_factor",
        //                   tsdf_surface_distance_threshold_factor_,
        //                   tsdf_surface_distance_threshold_factor_);
        // nh_private_.param("tsdf_world_frame", tsdf_world_frame_,
        // tsdf_world_frame_); nh_private_.param("tsdf_voxel_ply_output_path",
        // tsdf_voxel_ply_output_path_,
        //                   tsdf_voxel_ply_output_path_);
        // nh_private_.param("tsdf_mesh_output_path", tsdf_mesh_output_path_,
        //                   tsdf_mesh_output_path_);
        node_ptr_->declare_parameter("tsdf_surface_distance_threshold_factor",
                                     tsdf_surface_distance_threshold_factor_);
        node_ptr_->declare_parameter("tsdf_world_frame", tsdf_world_frame_);
        node_ptr_->declare_parameter("tsdf_voxel_ply_output_path",
                                     tsdf_voxel_ply_output_path_);
        node_ptr_->declare_parameter("tsdf_mesh_output_path",
                                     tsdf_mesh_output_path_);
        node_ptr_->get_parameter("tsdf_surface_distance_threshold_factor",
                                 tsdf_surface_distance_threshold_factor_);
        node_ptr_->get_parameter("tsdf_world_frame", tsdf_world_frame_);
        node_ptr_->get_parameter("tsdf_voxel_ply_output_path",
                                 tsdf_voxel_ply_output_path_);
        node_ptr_->get_parameter("tsdf_mesh_output_path",
                                 tsdf_mesh_output_path_);

        std::string color_mode = "color";
        // nh_private_.param("tsdf_mesh_color_mode", color_mode, color_mode);
        node_ptr_->declare_parameter("tsdf_mesh_color_mode", color_mode);
        node_ptr_->get_parameter("tsdf_mesh_color_mode", color_mode);
        if (color_mode == "color")
        {
            tsdf_mesh_color_mode_ = ColorMode::kColor;
        }
        ...
        else
        {
            ...
            // ros::shutdown();
            RCLCPP_FATAL_STREAM(node_ptr_->get_logger(),
                                "Undefined mesh coloring mode: " << color_mode);
            rclcpp::shutdown();
        }

        // ros::spinOnce();
        rclcpp::spin_some(node_ptr_->shared_from_this());
    }
...
void SimpleTsdfVisualizer::run(const Layer<TsdfVoxel> &tsdf_layer)
{
    ...
    RCLCPP_INFO_STREAM(node_ptr_->get_logger(), "\tVisualize all voxels...");
    {
        pcl::PointCloud<pcl::PointXYZI> pointcloud;
    ...
    RCLCPP_INFO_STREAM(node_ptr_->get_logger(), "\tVisualize mesh...");
    {
        ...
        mesh_integrator.reset(new MeshIntegrator<TsdfVoxel>(mesh_config,
                                                            tsdf_layer,
                                                            mesh_layer.get()));

        constexpr bool kOnlyMeshUpdatedBlocks = false;
        constexpr bool kClearUpdatedFlag      = false;
        mesh_integrator->generateMesh(kOnlyMeshUpdatedBlocks,
                                      kClearUpdatedFlag);
        ...
        sensor_msgs::msg::PointCloud2 pointcloud_msg;
        pcl::toROSMsg(pointcloud, pointcloud_msg);
        mesh_pointcloud_pub_->publish(pointcloud_msg);

        // Output as pcl mesh.
        pcl::PolygonMesh polygon_mesh;
        toConnectedPCLPolygonMesh(*mesh_layer,
                                  tsdf_world_frame_,
                                  &polygon_mesh);
        pcl_msgs::msg::PolygonMesh pcl_mesh_msg;
        pcl_conversions::fromPCL(polygon_mesh, pcl_mesh_msg);
        mesh_msg.header.stamp = node_ptr_->now();

        mesh_pcl_mesh_pub_->publish(pcl_mesh_msg);

        if (!tsdf_mesh_output_path_.empty())
        {
            if (voxblox::outputMeshLayerAsPly(tsdf_mesh_output_path_,
                                              *mesh_layer))
            {
                // ROS_INFO_STREAM("Output mesh PLY file to " <<
                // tsdf_mesh_output_path_);
                RCLCPP_INFO_STREAM(node_ptr_->get_logger(),
                                   "Output mesh PLY file to "
                                       << tsdf_mesh_output_path_);
            }
        }
    }
...
int main(int argc, char **argv)
{
    ...
    if (!voxblox::io::LoadLayer<voxblox::TsdfVoxel>(tsdf_proto_path,
                                                    &tsdf_layer))
    {
        // ROS_FATAL_STREAM("Unable to load a TSDF grid from: " <<
        // tsdf_proto_path);
        RCLCPP_FATAL_STREAM(
            node->get_logger(),
    ...
    voxblox::SimpleTsdfVisualizer visualizer(node.get());
