...
using ShouldVisualizeVoxelColorFunctionType = std::function<
    bool(const VoxelType &voxel, const Point &coord, Color *color)>;
 ...
 */
template <typename VoxelType>
using ShouldVisualizeVoxelIntensityFunctionType = std::function<
    bool(const VoxelType &voxel, const Point &coord, double *intensity)>;
    ...
    pcl::PointCloud<pcl::PointXYZRGB>                      *pointcloud)
{
    ...
    for (const BlockIndex &index : blocks)
    {
             ...
             ++linear_index)
        {
            ...
            if (vis_function(block.getVoxelByLinearIndex(linear_index),
                             coord,
                             &color))
            {
    ...
    for (const BlockIndex &index : blocks)
    {
             ...
             ++linear_index)
        {
            ...
            if (vis_function(block.getVoxelByLinearIndex(linear_index), coord))
            {
                ...
                colorVoxbloxToMsg(rainbowColorMap((coord.z() + 2.5) / 5.0),
                                  &color_msg);
...
inline bool visualizeTsdfVoxels(const TsdfVoxel &voxel,
                                const Point & /*coord*/,
                                Color *color)
{
    CHECK_NOTNULL(color);
...
inline bool
    visualizeDistanceIntensityTsdfVoxelsNearSurface(const TsdfVoxel &voxel,
                                                    const Point & /*coord*/,
                                                    double  surface_distance,
                                                    double *intensity)
{
...
inline bool
    visualizeDistanceIntensityTsdfVoxelsSlice(const TsdfVoxel &voxel,
                                              const Point     &coord,
                                              unsigned int     free_plane_index,
                                              float            free_plane_val,
                                              float            voxel_size,
                                              double          *intensity)
{
    CHECK_NOTNULL(intensity);
...
inline bool
    visualizeDistanceIntensityEsdfVoxelsSlice(const EsdfVoxel &voxel,
                                              const Point     &coord,
                                              unsigned int     free_plane_index,
                                              float            free_plane_val,
                                              float            voxel_size,
    ...
    pcl::PointCloud<pcl::PointXYZRGB> *pointcloud)
{
    ...
    createColorPointcloudFromLayer<TsdfVoxel>(
        layer,
        std::bind(&visualizeNearSurfaceTsdfVoxels,
                  ph::_1,
                  ph::_2,
                  surface_distance,
                  ph::_3),
 ...
 */
inline void
    createPointcloudFromTsdfLayer(const Layer<TsdfVoxel>            &layer,
                                  pcl::PointCloud<pcl::PointXYZRGB> *pointcloud)
{
    CHECK_NOTNULL(pointcloud);
    createColorPointcloudFromLayer<TsdfVoxel>(layer,
                                              &visualizeTsdfVoxels,
                                              pointcloud);
}
    ...
    pcl::PointCloud<pcl::PointXYZI> *pointcloud)
{
    ...
    createColorPointcloudFromLayer<TsdfVoxel>(
        layer,
        std::bind(&visualizeDistanceIntensityTsdfVoxelsNearSurface,
                  ph::_1,
                  ph::_2,
                  surface_distance,
                  ph::_3),
    ...
    pcl::PointCloud<pcl::PointXYZI> *pointcloud)
{
    ...
    createColorPointcloudFromLayer<EsdfVoxel>(
        layer,
        &visualizeDistanceIntensityEsdfVoxels,
        pointcloud);
}
    ...
    pcl::PointCloud<pcl::PointXYZI> *pointcloud)
{
    ...
    createColorPointcloudFromLayer<EsdfVoxel>(
        layer,
        std::bind(&visualizeFreeEsdfVoxels,
                  ph::_1,
                  ph::_2,
                  min_distance,
                  ph::_3),
...
{
    CHECK_NOTNULL(pointcloud);
    createColorPointcloudFromLayer<IntensityVoxel>(layer,
                                                   &visualizeIntensityVoxels,
                                                   pointcloud);
}
    ...
    pcl::PointCloud<pcl::PointXYZI> *pointcloud)
{
    ...
    // Using std::remainder rather than std::fmod as the latter has huge
    // crippling issues with floating-point division.
    ...
    createColorPointcloudFromLayer<TsdfVoxel>(
        layer,
        std::bind(&visualizeDistanceIntensityTsdfVoxelsSlice,
                  ph::_1,
                  ph::_2,
                  free_plane_index,
                  free_plane_val,
    ...
    pcl::PointCloud<pcl::PointXYZI> *pointcloud)
{
    ...
    createColorPointcloudFromLayer<EsdfVoxel>(
        layer,
        std::bind(&visualizeDistanceIntensityEsdfVoxelsSlice,
                  ph::_1,
                  ph::_2,
                  free_plane_index,
                  free_plane_val,
    ...
    visualization_msgs::msg::MarkerArray *marker_array)
{
    ...
    createOccupancyBlocksFromLayer<TsdfVoxel>(
        layer,
        std::bind(visualizeOccupiedTsdfVoxels,
                  std::placeholders::_1,
                  std::placeholders::_2,
                  layer.voxel_size()),
        frame_id,
...
inline void createOccupancyBlocksFromOccupancyLayer(
    const Layer<OccupancyVoxel>          &layer,
    const std::string                    &frame_id,
    visualization_msgs::msg::MarkerArray *marker_array)
{
    CHECK_NOTNULL(marker_array);
    createOccupancyBlocksFromLayer<OccupancyVoxel>(
        layer,
        &visualizeOccupiedOccupancyVoxels,
        frame_id,
        marker_array);
}
