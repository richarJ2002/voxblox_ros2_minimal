...
{
  public:
    SkeletonEvalNode(rclcpp::Node::SharedPtr node_ptr);
    ...
    bool selectRandomFreePose(const Point &min_bound,
...
SkeletonEvalNode::SkeletonEvalNode(rclcpp::Node::SharedPtr node_ptr) :
    node_ptr_(node_ptr),
    frame_id_("world"),
    visualize_(true),
    ...
    voxblox_server_(node_ptr.get())
{
    ...
    node_ptr_->get_parameter("full_euclidean_distance",
                             full_euclidean_distance_);
    node_ptr_->get_parameter("esdf_max_distance", esdf_max_distance_);
    node_ptr_->get_parameter("tsdf_max_distance", tsdf_max_distance_);
    node_ptr_->get_parameter("apply_noise", apply_noise_);
    node_ptr_->get_parameter("noise_sigma", noise_sigma_);
...
void SkeletonEvalNode::generateWorld()
{
    ...
    world_.addObject(std::unique_ptr<voxblox::Object>(
        new voxblox::Cube(Point(3.75, 5.0, 2.5),
                          Point(7.5, 0.5, 5.0),
                          voxblox::Color::Gray())));
    world_.addObject(std::unique_ptr<voxblox::Object>(
        new voxblox::Cube(Point(9.5, 5.0, 2.5),
                          Point(1, 0.5, 5.0),
                          voxblox::Color::Gray())));

    world_.addObject(std::unique_ptr<voxblox::Object>(
        new voxblox::Cube(Point(1.25, 7.5, 2.5),
                          Point(2.5, 0.5, 5.0),
                          voxblox::Color::Gray())));
    world_.addObject(std::unique_ptr<voxblox::Object>(
        new voxblox::Cube(Point(6.5, 7.5, 2.5),
                          Point(5, 0.5, 5.0),
                          voxblox::Color::Gray())));
    world_.addObject(std::unique_ptr<voxblox::Object>(
        new voxblox::Cube(Point(12.5, 7.5, 2.5),
                          Point(5, 0.5, 5.0),
                          voxblox::Color::Gray())));

    world_.addObject(std::unique_ptr<voxblox::Object>(
        new voxblox::Cube(Point(10, 2.5, 2.5),
                          Point(0.5, 5.0, 5.0),
                          voxblox::Color::Gray())));
    world_.addObject(std::unique_ptr<voxblox::Object>(
        new voxblox::Cube(Point(8, 8.75, 2.5),
                          Point(0.5, 2.5, 5.0),
                          voxblox::Color::Gray())));
    world_.addObject(std::unique_ptr<voxblox::Object>(
        new voxblox::Cube(Point(5, 8.25, 2.5),
                          Point(0.5, 1.5, 5.0),
                          voxblox::Color::Gray())));

    // Random objects.
    world_.addObject(std::unique_ptr<voxblox::Object>(
        new voxblox::Cube(Point(2.5, 2.5, 2.5),
                          Point(1.0, 1.0, 5.0),
                          voxblox::Color::Gray())));
    world_.addObject(std::unique_ptr<voxblox::Object>(
        new voxblox::Cube(Point(3.5, 2.5, 1.25),
                          Point(1.0, 1.0, 2.5),
                          voxblox::Color::Green())));
    world_.addObject(std::unique_ptr<voxblox::Object>(
        new voxblox::Cube(Point(13.75, 8.5, 1.25),
                          Point(2.5, 2.0, 2.5),
                          voxblox::Color::Green())));

    world_.addObject(std::unique_ptr<voxblox::Object>(
        new voxblox::Cylinder(Point(12.5, 5.0, 1.25),
                              0.5,
                              2.5,
                              voxblox::Color::Pink())));
    world_.addObject(std::unique_ptr<voxblox::Object>(
        new voxblox::Sphere(Point(11.5, 1.5, 1.25),
                            1.0,
                            voxblox::Color::Pink())));
    world_.addObject(std::unique_ptr<voxblox::Object>(
        new voxblox::Sphere(Point(13.5, 1.5, 1.25),
                            1.0,
                            voxblox::Color::Pink())));
}
...
void SkeletonEvalNode::transformPointcloud(
    const voxblox::Transformation &T_N_O,
    const voxblox::Pointcloud     &ptcloud,
    voxblox::Pointcloud           *ptcloud_out) const
{
    ptcloud_out->clear();
...
void SkeletonEvalNode::generateMapFromRobotPoses(int num_poses, int seed)
{
    ...
    for (int i = 0; i < num_poses; i++)
    {
        ...
        if (!apply_noise_)
        {
            world_.getPointcloudFromViewpoint(view_origin,
                                              view_direction,
                                              camera_resolution_,
                                              camera_fov_h_rad_,
                                              camera_max_dist_,
        ...
        {
            world_.getNoisyPointcloudFromViewpoint(view_origin,
                                                   view_direction,
                                                   camera_resolution_,
                                                   camera_fov_h_rad_,
                                                   camera_max_dist_,
...
void SkeletonEvalNode::generateSkeleton()
{
    ...
    float min_separation_angle = skeleton_generator.getMinSeparationAngle();
    ...
    // nh_private_.param("min_gvd_distance", min_gvd_distance,
    // min_gvd_distance); node_ptr_->declare_parameter("min_gvd_distance",
    // min_gvd_distance);
    ...
    RCLCPP_INFO(
        ...
        "Map voxel size: %f Noise on? %d Noise level: %f",
        voxblox_server_.getTsdfMapPtr()->getTsdfLayerPtr()->voxel_size(),
        apply_noise_,
        noise_sigma_);
    RCLCPP_INFO(
...
int main(int argc, char **argv)
{
    ...
    voxblox::SkeletonEvalNode node(nh);
