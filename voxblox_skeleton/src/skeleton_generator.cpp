...
void SkeletonGenerator::setEsdfLayer(Layer<EsdfVoxel> *esdf_layer)
{
    ...
    // Make a skeleton layer to store the intermediate skeleton steps, along
    // with the lists.
    skeleton_layer_.reset(
        new Layer<SkeletonVoxel>(esdf_layer_->voxel_size(),
    ...
    for (const BlockIndex &block_index : blocks)
    {
             ...
             ++lin_index)
        {
            ...
            if (!esdf_voxel.observed ||
                esdf_voxel.distance < min_gvd_distance_ || esdf_voxel.fixed)
            {
                continue;
            }
            ...
            // Get the floating-point distance of this voxel, normalize it as
            // long as it's not 0.
            ...
            Neighborhood<>::getFromBlockAndVoxelIndex(block_index,
                                                      voxel_index,
                                                      voxels_per_side_,
                                                      &neighbors);
            ...
            for (size_t i = 0; i < neighbors.size(); ++i)
            {
                ...
                // Get the relative distance: that is, if this voxel was on top
                // of ours, what direction would it point in, roughly? Make sure
                // this is a double so we can normalize it.
                ...
                {
                    // This is pointing at us! We're its parent. No way is this
                    // a skeleton point.
                    continue;
                }
                ...
                if (acos(dot_prod) >= min_separation_angle_)
                {
                    ...
                    skeleton_point.basis_directions.push_back(
                        relative_direction);
                }
            }
            if (on_skeleton)
            {
                ...
                skeleton_voxel.num_basis_points =
                    skeleton_point.num_basis_points;
                if (!generate_by_layer_neighbors_)
                {
                    skeleton_voxel.is_face =
                        (skeleton_voxel.num_basis_points == 9);
                    skeleton_voxel.is_edge =
                        (skeleton_voxel.num_basis_points >= 12);
                    skeleton_voxel.is_vertex =
                        (skeleton_voxel.num_basis_points == 16);
                ...
                {
                    // If we're generating by layer neighbors, don't bother
                    // setting vertex vs. edge yet.
...
void SkeletonGenerator::generateSparseGraph()
{
    ...
    const float grid_size_inv = 1.0 / voxel_size_;
    ...
    while (!floodfill_queue.empty())
    {
        ...
        {
            // If it's an edge, check if it's already checked (i.e., colored by
            // us). There's no way we should have to re-color or merge subgraphs
            // if we are doing flood-fill correctly so only 1 case here.
            ...
            {
                // Otherwise mark this thing as visited and throw its neighbors
                // into the fray.
        ...
        if (connected_vertex_id >= 0)
        {
            ...
            // TODO(helenol): some more checks, for instance merge directly
            // adjacent vertices. Before adding an edge, make sure it's not
            // already in there...
...
void SkeletonGenerator::octreeLabeling(int               octant,
                                       int               label,
...
void SkeletonGenerator::pruneDiagramVertices()
{
    ...
    for (size_t i = 0; i < num_vertices; ++i)
    {
        ...
        std::vector<std::pair<size_t, float>> returned_matches;
        nanoflann::SearchParams               params; // Defaults are fine.

        size_t num_matches = kd_tree.radiusSearch(
            const_vertices[i].point.data(),
            vertex_pruning_radius_ * vertex_pruning_radius_,
            returned_matches,
            params);

        // Go through all the matches... Figure out if we actually need to
        // delete something. Keep track of our favorite to keep.
        size_t num_valid_matches       = 0;
        float  largest_vertex_distance = const_vertices[i].distance;
        size_t favorite_vertex_index   = i;
        for (const std::pair<size_t, float> &match : returned_matches)
        {
            if (match.first == i)
            {
                continue;
    ...
    const std::vector<int64_t> &starting_edge_ids)
{
    ...
    const float kMaxThreshold = 2 * voxel_size_;
    const float kVertexSearchRadus =
        vertex_pruning_radius_ * vertex_pruning_radius_;
    ...
    for (size_t i = 0; i < edge_ids.size(); ++i)
    {
        ...
        float                max_d = getMaxEdgeDistanceFromStraightLine(start,
                                                         end,
                                                         &coordinate_path,
                                                         &max_d_ind);
        ...
        {
            // Pick the point with the max deviation to insert a new voxel
            // there.
            ...
            // I guess it would be best if this was a dynamic k-D tree, since
            // we're adding/subtracting vertices at will. Also not necessarily
            // indexed in order. Pair from index and distance.
            std::vector<std::pair<size_t, float>> returned_matches;
            nanoflann::SearchParams               params; // Defaults are fine.
            ...
            if (squared_distance < kVertexSearchRadus)
            {
                ...
                if (iter != graph_.getVertexMap().end())
                {
                    ...
                    {
                        if (graph_.areVerticesDirectlyConnected(
                                vertex_candidate.vertex_id,
                                edge.start_vertex) &&
                            graph_.areVerticesDirectlyConnected(
                                vertex_candidate.vertex_id,
                                edge.end_vertex))
                        {}
                        else
                        {
                            // Try to find a connection from start vertex ->
                            // this and then from this to end vertex.
                            AlignedVector<Point> start_path, end_path;

                            timing::Timer path_timer(
                                "skeleton/split_edges/search_path");
                            bool success_start =
                                skeleton_planner_.getPathOnDiagram(
                                    start,
                                    vertex_candidate.point,
                                    &start_path);
                            bool success_end =
                                skeleton_planner_.getPathOnDiagram(
                                    vertex_candidate.point,
                            ...
                            if (success_start && success_end)
                            {
                                ...
                                float  max_d_start = getMaxEdgeDistanceOnPath(
                                    start,
                                    vertex_candidate.point,
                                    start_path,
                                    &max_start_index);
                                float max_d_end = getMaxEdgeDistanceOnPath(
                                    vertex_candidate.point,
                                    end,
                                    end_path,
                                    &max_end_index);
                                ...
                                {
                                    // Only connect to this if it ACTUALLY
                                    // lowers the costs!

                                    // Remove the existing edge, add two new
                                    // edges.
                                    SkeletonEdge new_edge_1, new_edge_2;
                                    new_edge_1.start_vertex = edge.start_vertex;
                                    new_edge_1.end_vertex =
                                        vertex_candidate.vertex_id;
                                    new_edge_2.start_vertex =
                                        vertex_candidate.vertex_id;
                                    new_edge_2.end_vertex = edge.end_vertex;

                                    int64_t edge_id_1 =
                                        graph_.addEdge(new_edge_1);
                                    int64_t edge_id_2 =
                                        graph_.addEdge(new_edge_2);

                                    // Make sure two new edges are going to be
                                    // iteratively checked again.
            ...
            {
                if (voxel == nullptr ||
                    (voxel->is_vertex && voxel->vertex_id > -1))
                {
                    continue;
                }
            }
            ...
            {
                timing::Timer kdtree_add_timer(
                    "skeleton/split_edges/kdtree_add");
...
float SkeletonGenerator::getMaxEdgeDistanceFromStraightLine(
    const Point          &start,
    const Point          &end,
    AlignedVector<Point> *coordinate_path,
    size_t               *max_index)
...
float SkeletonGenerator::getMaxEdgeDistanceOnPath(
    const Point                &start,
    const Point                &end,
...
void SkeletonGenerator::repairGraph()
{
    ...
    for (const std::pair<int, int64_t> &subgraph1 : subgraph_vertex_examples)
    {
        ...
        for (const std::pair<int, int64_t> &subgraph2 :
             subgraph_vertex_examples)
        {
            ...
            // Ok now presumably we have two different subgraphs that we're
            // gonna try to connect.
            AlignedVector<Point> coordinate_path;
            bool success = skeleton_planner_.getPathOnDiagram(vertex1.point,
                                                              vertex2.point,
                                                              &coordinate_path);
            if (success)
            {
                tryToFindEdgesInCoordinatePath(coordinate_path,
                                               subgraph1.first,
                                               subgraph2.first,
                                               &new_edge_ids);
            }
    ...
    for (const Point &coord : coordinate_path)
    {
        ...
        if (voxel->is_vertex && voxel->vertex_id >= 0)
        {
            ...
            if (graph_.hasVertex(vertex_id))
            {
                ...
                if (vertex.subgraph_id > 0)
                {
                    // If this is the same subgraph ID as the previous one, then
                    // just store it as the latest.
                    ...
                    {
                        // Ok this is a different subgraph! Let's put in an
                        // edge.
                        ...
                        // Probably the right time to recursively label stuff
                        // too.
...
void SkeletonGenerator::simplifyVertices()
{
    ...
    std::vector<float>      out_dist_sqr(kNumNeighbors);
    ...
    for (const int64_t vertex_id : vertex_ids)
    {
        ...
        if (vertex.edge_list.size() == 1)
        {
            ...
            // kD tree lookup here.
            size_t num_results = kd_tree.knnSearch(vertex.point.data(),
                                                   kNumNeighbors,
                                                   &ret_index[0],
                                                   &out_dist_sqr[0]);
            for (size_t i = 0; i < num_results; i++)
            {
                const SkeletonVertex &neighbor_vertex =
                    graph_.getVertex(ret_index[i]);
                if (neighbor_vertex.edge_list.size() == 1)
                {
                    timing::Timer path_timer(
                        "skeleton/simplify_vertices/path_finding");

                    // We don't want stuff that's easily linked in the diagram,
                    // or we would have already gotten this.
                    AlignedVector<Point> coordinate_path;
                    ...
                    success =
                        skeleton_planner_.getPathInEsdf(vertex.point,
                                                        neighbor_vertex.point,
                                                        &coordinate_path);
                    ...
                    if (success)
                    {
                        size_t max_index;
                        float  max_d =
                            getMaxEdgeDistanceOnPath(vertex.point,
                                                     neighbor_vertex.point,
                                                     coordinate_path,
                                                     &max_index);
                        if (max_d > kMaxThreshold)
    ...
    for (const int64_t vertex_id : vertex_ids)
    {
        ...
        if (vertex.edge_list.size() == 2)
        {
            ...
            AlignedVector<Point> coordinate_path;
            bool success = skeleton_planner_.getPathInEsdf(vertex1.point,
                                                           vertex2.point,
                                                           &coordinate_path);
            if (!success)
            {
            ...
            size_t max_index;
            float  max_d = getMaxEdgeDistanceOnPath(vertex1.point,
                                                   vertex2.point,
                                                   coordinate_path,
                                                   &max_index);
    ...
    for (const int64_t vertex_id : vertex_ids)
    {
        ...
        for (size_t i = 0; i < num_results; i++)
        {
            ...
            if (success)
            {
                ...
                mergeSubgraphs(vertex.subgraph_id,
                               neighbor_vertex.subgraph_id,
                               &subgraph_map);
            }
            path_timer.Stop();
        }
...
void SkeletonGenerator::mergeSubgraphs(int                 subgraph_1,
                                       int                 subgraph_2,
