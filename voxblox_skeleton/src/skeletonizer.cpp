...
void SkeletonizerNode::init()
{
    ...
    if (update_esdf || esdf_server_.getEsdfMapPtr()
                               ->getEsdfLayerPtr()
    ...
    RCLCPP_INFO(node_ptr_->get_logger(), "Finished updating ESDF.");
    ...
    if (!sparse_graph_filepath.empty())
    {
        ...
        {
            RCLCPP_INFO(node_ptr_->get_logger(),
                        "Output sparse graph to: %s",
                        sparse_graph_filepath.c_str());
        }
        else
        {
            RCLCPP_ERROR(node_ptr_->get_logger(),
                         "Couldn't output sparse graph to: %s",
                         sparse_graph_filepath.c_str());
        }
    }
}
