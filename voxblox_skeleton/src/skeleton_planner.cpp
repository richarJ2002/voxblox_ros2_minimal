...
template <>
inline Block<SkeletonVoxel>::ConstPtr
    SkeletonAStar::getBlockPtrByIndex<SkeletonVoxel>(
                                  ...
                                  AlignedVector<Point> *coordinate_path) const
{
    ...
    Point start_position_center =
        getBlockAndVoxelIndex<EsdfVoxel>(start_position,
                                         &start_block_index,
                                         &start_voxel_index);
    getBlockAndVoxelIndex<EsdfVoxel>(end_position,
                                     &end_block_index,
                                     &end_voxel_index);

    // Get the distance to the goal index.
    Eigen::Vector3i goal_voxel_offset =
        Neighborhood<>::getOffsetBetweenVoxels(start_block_index,
                                               start_voxel_index,
                                               end_block_index,
                                               end_voxel_index,
    ...
    bool success = getPathInVoxels<EsdfVoxel>(start_block_index,
                                              start_voxel_index,
                                              goal_voxel_offset,
                                              &voxel_path);
    ...
    AlignedVector<Point> *coordinate_path) const
{
    ...
    Point start_position_center =
        getBlockAndVoxelIndex<SkeletonVoxel>(start_position,
                                             &start_block_index,
                                             &start_voxel_index);
    getBlockAndVoxelIndex<SkeletonVoxel>(end_position,
                                         &end_block_index,
                                         &end_voxel_index);
    ...
    // Now get the path in voxels back out.
    AlignedVector<Eigen::Vector3i> voxel_path;
    bool success = getPathInVoxels<SkeletonVoxel>(start_block_index,
                                                  start_voxel_index,
                                                  goal_voxel_offset,
                                                  &voxel_path);
    ...
    AlignedVector<Point> *coordinate_path) const
{
    ...
    if (!getPathToNearestDiagramPt(start_block_index,
                                   start_voxel_index,
                                   goal_voxel_offset,
                                   &voxel_path_to_start))
    {
    ...
    if (!getPathToNearestDiagramPt(end_block_index,
                                   end_voxel_index,
                                   -goal_voxel_offset,
                                   &voxel_path_from_end))
    {
    ...
    Neighborhood<>::getFromBlockAndVoxelIndexAndDirection(
        start_block_index,
        start_voxel_index,
        voxel_path_to_start.back(),
        voxels_per_side,
        &diagram_start_block_index,
        &diagram_start_voxel_index);
    Neighborhood<>::getFromBlockAndVoxelIndexAndDirection(
        end_block_index,
        end_voxel_index,
        voxel_path_from_end.back(),
        voxels_per_side,
        &diagram_end_block_index,
        &diagram_end_voxel_index);

    goal_voxel_offset =
        Neighborhood<>::getOffsetBetweenVoxels(diagram_start_block_index,
                                               diagram_start_voxel_index,
                                               diagram_end_block_index,
                                               diagram_end_voxel_index,
                                               voxels_per_side);

    if (!getPathInVoxels<SkeletonVoxel>(diagram_start_block_index,
                                        diagram_start_voxel_index,
                                        goal_voxel_offset,
                                        &voxel_path_on_diagram))
    {
    ...
    voxelPathToCoordinatePath(coordinate_path_start.back(),
                              skeleton_layer_->voxel_size(),
                              voxel_path_on_diagram,
                              &coordinate_path_diagram);
    voxelPathToCoordinatePath(end_position_center,
                              skeleton_layer_->voxel_size(),
                              voxel_path_from_end,
                              &coordinate_path_end);
    ...
    AlignedVector<Point>                 *coordinate_path) const
{
    ...
    for (const Eigen::Vector3i &voxel_offset : voxel_path)
    {
        coordinate_path->push_back(start_location +
                                   voxel_offset.cast<float>() * voxel_size);
    }
}

Eigen::Vector3i
    SkeletonAStar::popSmallestFromOpen(const IndexToDistanceMap &f_score_map,
...
void SkeletonAStar::getSolutionPath(
    const Eigen::Vector3i          &end_voxel_offset,
    ...
    AlignedVector<Eigen::Vector3i> *voxel_path) const
{
    ...
    while (!open_set.empty())
    {
        ...
        if (isValidVoxel<SkeletonVoxel>(block_index,
                                        voxel_index,
                                        block_index,
                                        skeleton_block_ptr))
        {
        ...
        for (size_t i = 0; i < neighbors.size(); ++i)
        {
            ...
            if (!isValidVoxel<EsdfVoxel>(neighbor_block_index,
            ...
            float tentative_g_score = g_score_map[current_voxel_offset] +
                                      Neighborhood<>::kDistances(i);
                ...
                g_score_map[neighbor_voxel_offset] < tentative_g_score)
            {
                ...
                f_score_map[neighbor_voxel_offset] =
                    tentative_g_score +
                    estimateCostToGoal(neighbor_voxel_offset,
                                       goal_voxel_offset);
