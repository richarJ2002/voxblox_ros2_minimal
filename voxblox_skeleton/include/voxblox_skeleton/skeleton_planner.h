#ifndef VOXBLOX_SKELETON_SKELETON_PLANNER_H_
#define VOXBLOX_SKELETON_SKELETON_PLANNER_H_
...
{
  public:
    typedef typename AnyIndexHashMapType<float>::type IndexToDistanceMap;
    typedef
        typename AnyIndexHashMapType<Eigen::Vector3i>::type IndexToParentMap;
    ...
    inline void setEsdfLayer(const Layer<EsdfVoxel> *esdf_layer)
    {
        ...
        if (skeleton_layer_ == nullptr)
        {
            ...
            skeleton_layer_ =
                new Layer<SkeletonVoxel>(esdf_layer->voxel_size(),
                                         esdf_layer->voxels_per_side());
        }
    }
    ...
    float getMinEsdfDistance() const
    {
        return min_esdf_distance_;
    }
    ...
    bool getPathInEsdf(const Point          &start_position,
    ...
    bool
        getPathUsingEsdfAndDiagram(const Point          &start_position,
                                   const Point          &end_position,
                                   AlignedVector<Point> *coordinate_path) const;

    // Requires that the start and end points are on the diagram.
    bool getPathOnDiagram(const Point          &start_position,
                          const Point          &end_position,
                          AlignedVector<Point> *coordinate_path) const;
    ...
    float estimateCostToGoal(const Eigen::Vector3i &voxel_offset,
  ...
  private:
    ...
    template <typename VoxelType>
    typename Block<VoxelType>::ConstPtr
        getBlockPtrByIndex(const BlockIndex &block_index) const;

    // Calculate what the block and voxel indices should be based on different
    // layer types.
    template <typename VoxelType>
    Point getBlockAndVoxelIndex(const Point &coord,
    ...
    AlignedVector<Eigen::Vector3i> *voxel_path) const
{
    ...
    while (!open_set.empty())
    {
        ...
        for (size_t i = 0; i < neighbors.size(); ++i)
        {
            ...
            if (!isValidVoxel<VoxelType>(neighbor_block_index,
...
#endif // VOXBLOX_SKELETON_SKELETON_PLANNER_H_
