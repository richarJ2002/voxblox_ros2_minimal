...
class SkeletonGenerator
{
  public:
    ...
    SparseSkeletonGraph &getSparseGraph()
    {
        return graph_;
    }

    float getMinSeparationAngle() const
    {
        return min_separation_angle_;
    }
    ...
    float getMinGvdDistance() const
    {
        return min_gvd_distance_;
    }
    void setMinGvdDistance(float min_gvd_distance)
    {
        min_gvd_distance_ = min_gvd_distance;
        skeleton_planner_.setMinEsdfDistance(min_gvd_distance_);
    }

    int getNumNeighborsForEdge() const
    {
        return num_neighbors_for_edge_;
    }
    ...
    Layer<SkeletonVoxel> *getSkeletonLayer()
    {
    ...
    // Determine whether a point is an endpoint (using our own amazing
    // algorithm).
    bool isEndPoint(const std::bitset<27> &neighbors) const;

    // Utility functions for edge stuff.
    float getMaxEdgeDistanceFromStraightLine(
        const Point          &start,
        const Point          &end,
        AlignedVector<Point> *coordinate_path,
        size_t               *max_index);
    float getMaxEdgeDistanceOnPath(const Point                &start,
                                   const Point                &end,
    ...
    void mergeSubgraphs(int                 subgraph_1,
                        int                 subgraph_2,
  ...
  private:
    ...
    typedef nanoflann::KDTreeSingleIndexAdaptor<
        ...
        VertexGraphKdTree;
    typedef nanoflann::KDTreeSingleIndexAdaptor<
        nanoflann::L2_Simple_Adaptor<float, SkeletonPointVectorAdapter>,
        SkeletonPointVectorAdapter,
        3>
        SkeletonPointKdTree;
